...
/*export function fieldOfView(ox: number, oy: number, radius: number, visit: (x: number, y:number) => void, blocked: (x: number, y:number) => boolean) {
    visit(ox, oy); // origin always visited.

    function quadrant(dx: number, dy: number) {
        var light = new Light(radius);
        for (var dr = 1; dr <= radius; dr += 1) {
            for (var i = 0; i <= dr; i++) {
...
function Pt(x: number, y: number) { this.x = x; this.y = y; }
...
function Ln(p: number, q: number) { this.p = p; this.q = q; }

Ln.prototype.toString = function () { return this.p + '-' + this.q; }
Ln.prototype.copy = function () { return new Ln(this.p.copy(), this.q.copy()); }
Ln.prototype.cw = function (pt: {x: number, y: number }) { return this.dtheta(pt) > 0; }
Ln.prototype.ccw = function (pt: {x: number, y: number }) { return this.dtheta(pt) < 0; }
Ln.prototype.dtheta = function (pt: {x: number, y: number }) {
...
function Arc(steep: any, shallow: any) { // Ln & Ln
    this.steep = steep;
...
Arc.prototype.bumpCCW = function (pt: any) { // Pt
...
Arc.prototype.shade = function (pt: any) { //Pt
...
Light.prototype.hits = function (pt: any): any { //Pt
...
Light.prototype.shade = function (arci: any, pt: any) { // Arc ?? & Pt
